...
\paragraph{PRNG}È un algoritmo deterministico che usa il seme e una parte di elaborazione intermedia per generare un flusso pseudocasuale (è apparentemente casuale, ovvero distinguere un flusso di bit generato da un TRNG, o in generale da una sequenza veramente casualle di bit  da un flusso generato da un PRNG è estremamente difficile).
\paragraph{PRF}Le Pseudo Random Functions sono algoritmi deterministici che prendono in ingresso, un seme, una parte di elaborazione intermedia e valori specifici del contesto (i parametri di funzione) e restituiscono un valore pseudocasuale.
\paragraph{Message authentication Code} È un tag che permette di verificare l'integrità di un messaggio.  Per autenticare un messaggio si potrebbe utilizzare la crittografia simmetrica inserendo nel messaggio un \textbf{sequence number} per evitare il riordinamento e un \textbf{error detection code} per verificare che il messaggio non sia stato alterato. Il \textbf{timestamp} può essere messo per proteggere dai \textbf{replay attack}. Oppure il messaggio potrebbe non essere crittografato (non viene garantita l'integrità). Si aggiunge un MAC. Per creare i MAC si usano gli algoritmi MAC (spesso funzioni hash). Il MAC viene aggiunto al messaggio. Chi riceve il messaggio utilizzerà lo stesso algoritmo MAC per calcolarlo sul messaggio e confrontarlo con il MAC ricevuto.
...
\paragraph{Diffie-Hellman e il meccanismo di scambio delle chiavi pubbliche}
\begin{figure}[thbp]
	\centering
	...
	\label{fig:dh}
\end{figure}
	L'obbiettivo di \ul{distribuire le chiavi simmetriche} e \ul{come fare le firme digitali.} Diffie e Hellman proposero un'idea di algoritmo (\textbf{non un algoritmo!}) per fare la crittografia asimmetrica con due chiavi separate. In generale, più lunga è una chiave, più è sicura.  

	Permette di scambiare le chiavi segrete in modo sicuro (\cref{fig:dh}) basando tale meccanismo sul calcolo dei logaritmi discreti. È computazionalmente difficile calcolare il logaritmo discreto (calcolare l'esponente conoscendo la base e il numero davanti al modulo). 
	...
	\paragraph{RSA}Primo algoritmo ad implementare l'idea di Diffie e Hellman.  
